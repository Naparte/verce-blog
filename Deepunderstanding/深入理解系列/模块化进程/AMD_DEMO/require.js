void (function () {
  var initRequire = function () {
    ...
    var splitPathRegex = /^(.+\/(?!$)|\/)?((?:.+?)?(\.[^.]*)?)$/,
      getDir = function (path) {
        return splitPathRegex.exec(path)[1] || "";
      };
    ...
    var resolvePath = function (currentBase, searchPath) {
      ...
      return [
        normalize(getDir(currentBase) + "/" + searchPath),
        normalize(currentBase + "/" + searchPath),
      ];
    };
    ...
    window.require = function (modulePath) {
      ...
      for (var i = 0, path; (path = possiblePaths[i]); i++) {
        if (require._modules[path]) {
          return require._modules[path].exports;
        }
      }
      ...
      try {
        ...
        if (e instanceof SyntaxError) {
          log(foundPath, "a syntax error prevented execution");
        }
        throw e;
      }
      ...
      try {
        ...
        if (e.type == "syntax_error") {
          ...
          if (e.type == "stack_overflow") {
            ...
            log(
              foundPath,
              'enable "break on error" in your debugger to debug',
              e
            );
          }
          throw e;
  ...
  if (typeof module != "undefined" && module.exports) {
    ...
    require._evaluate =
      typeof eval("(function(){})") == "undefined"
        ? function (src, path) {
    ...
    void (function () {
      ...
      for (var i = 0, script; (script = scripts[i]); i++) {
        if (!script.src.match(requireSrcRegex)) {
          continue;
        }
