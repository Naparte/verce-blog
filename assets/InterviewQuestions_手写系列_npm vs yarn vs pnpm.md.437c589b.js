import{_ as s,o as n,c as a,Q as l}from"./chunks/framework.84f90f1e.js";const E=JSON.parse('{"title":"npm vs yarn vs pnpm","description":"","frontmatter":{},"headers":[],"relativePath":"InterviewQuestions/手写系列/npm vs yarn vs pnpm.md","filePath":"InterviewQuestions/手写系列/npm vs yarn vs pnpm.md","lastUpdated":1704035115000}'),p={name:"InterviewQuestions/手写系列/npm vs yarn vs pnpm.md"},e=l(`<h1 id="npm-vs-yarn-vs-pnpm" tabindex="-1">npm vs yarn vs pnpm <a class="header-anchor" href="#npm-vs-yarn-vs-pnpm" aria-label="Permalink to &quot;npm vs yarn vs pnpm&quot;">​</a></h1><blockquote><p>npm/yarn install 原理</p></blockquote><ul><li>主要分为两个部分, 首先，执行 npm/yarn install之后，包如何到达项目 node_modules 当中。其次，node_modules 内部如何管理依赖;执行命令后，首先会构建依赖树，然后针对每个节点下的包，会经历下面四个步骤:</li></ul><ol><li>将依赖包的版本区间解析为某个具体的版本号</li><li>下载对应版本依赖的 tar 包到本地离线镜像</li><li>将依赖从离线镜像解压到本地缓存</li><li>将依赖从缓存拷贝到当前目录的 node_modules 目录</li></ol><blockquote><p>依赖在node_modules内部目录结构</p></blockquote><div class="language-javascript vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">javascript</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#6A737D;">//  npm1、npm2 中呈现出的是嵌套结构</span></span>
...
<span class="line"><span style="color:#6A737D;">// 1. 依赖结构的不确定性(不确定性是指，foo和bar都依赖 base64的npm，但是版本不一致，谁先被提到node_modules下面的是不确定的)。</span></span>
<span class="line"><span style="color:#6A737D;">// 2. 扁平化算法本身的复杂性很高，耗时较长。</span></span>
<span class="line"><span style="color:#6A737D;">// 3. 项目中仍然可以非法访问没有声明过依赖的包</span></span></code></pre></div><ul><li>pnpm 结构 <ol><li>根目录下的 node_modules 下面不再是眼花缭乱的依赖，而是跟 package.json 声明的依赖基本保持一致。即使 pnpm 内部会有一些包会设置依赖提升，会被提升到根目录 node_modules 当中，但整体上，根目录的node_modules比以前还是清晰和规范了许多</li><li>外层的目录会映射到pnpm目录下的包里面</li></ol></li></ul><p><img src="https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/64c0cdbb6cf64187b9ca0c05b556f860~tplv-k3u1fbpfcp-watermark.image?" alt="image.png"></p><h2 id="总结" tabindex="-1">总结 <a class="header-anchor" href="#总结" aria-label="Permalink to &quot;总结&quot;">​</a></h2><blockquote><p>npm（包管理鼻祖）</p></blockquote><ul><li>优点</li></ul><ol><li>官方支持：npm 是 Node.js 的官方包管理器，因此它得到了广泛的支持和使用。</li><li>自动依赖项解析：npm 可以自动解析项目中的依赖项，并安装所需的软件包。</li><li>版本控制：npm 使用 package-lock.json 文件来确保安装过程中使用相同的依赖项版本。</li></ol><ul><li>缺点</li></ul><ol><li>安装速度较慢：由于 npm 是单线程解析依赖项，因此安装速度可能相对较慢。</li><li>文件冲突：由于 npm 使用 package-lock.json 文件来锁定依赖项版本，</li></ol><blockquote><p>yarn（解决npm不能并发和锁版本问题）</p></blockquote><ul><li>优点</li></ul><ol><li>安装速度快：由于 yarn 可以并行下载和缓存软件包，因此安装速度通常比 npm 快。</li><li>版本控制：yarn 使用 yarn.lock 文件来确保安装过程中使用相同的依赖项版本。</li><li>离线模式：yarn 支持离线模式，可以在没有互联网连接的情况下工作。<a href="https://www.yarnpkg.cn/features/offline-cache" target="_blank" rel="noreferrer">https://www.yarnpkg.cn/features/offline-cache</a></li></ol><blockquote><p>缺点</p></blockquote><ol><li>依赖项缓存：yarn 将所有软件包都缓存在本地，这可能会占用大量磁盘空间。</li></ol><blockquote><p>pnpm（解决幽灵依赖问题）</p></blockquote><ul><li>优点</li></ul><ol><li>安装速度最快：由于 pnpm 可以共享依赖项，因此它可以更快地安装和更新模块。</li><li>多线程解析：与 yarn 类似，pnpm 也可以使用多线程解析依赖项。</li><li>离线模式：pnpm 支持离线模式，并且可以在没有互联网连接的情况下工作。</li><li>更好的内存管理：pnpm 使用更好的内存管理技术，可以更有效地利用系统资源。</li></ol><ul><li>缺点</li></ul><ol><li>相对较新：由于 pnpm 是较新的包管理器，因此可能会缺乏一些 npm 和 yarn 中的功能和资源。</li><li>可能存在兼容性问题：由于 pnpm 采用了不同的依赖项解析方法，因此可能会存在一些兼容性问题。</li></ol><p><em>参考资料</em>: 链接：<a href="https://juejin.cn/post/7232114747347484709" target="_blank" rel="noreferrer">https://juejin.cn/post/7232114747347484709</a> 链接：<a href="https://juejin.cn/post/6932046455733485575" target="_blank" rel="noreferrer">https://juejin.cn/post/6932046455733485575</a></p>`,26),o=[e];function c(i,t,r,d,m,y){return n(),a("div",null,o)}const _=s(p,[["render",c]]);export{E as __pageData,_ as default};
